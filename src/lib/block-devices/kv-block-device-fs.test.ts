...
import { KvBlockDeviceFs } from './kv-block-device-fs';
...
describe('KvBlockDeviceFs', () => {
    let tmpDir: string;
    let device: KvBlockDeviceFs;

    beforeEach(() => {
        tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'kv-fs-test-'));
        device = new KvBlockDeviceFs(BLOCK_SIZE, tmpDir);
    });
