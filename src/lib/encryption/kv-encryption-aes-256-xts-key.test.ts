...
describe('KvEncryptionAES256XTSKey', () => {
    ...
    // (run with raw Node, not Bun) using `createCipheriv('aes-256-xts', ...)`
    // — Node ships the standard XTS cipher; our pure-JS reimplementation on
    // top of AES-256-ECB must match it byte-for-byte.
    describe('hard-coded vector', () => {
        ...
        const CIPHERTEXT_HEX = '589261f49c3ab549267654518a8ec5ca3c78eeca3b319f94d4e5e87cce6170dd';
        ...
        it('encrypts a known plaintext to the expected ciphertext bytes', async () => {
            const cipher = new KvEncryptionAES256XTSKey(fromHex(KEY_HEX));
            const out = await cipher.encrypt(BLOCK_ID, fromHex(PLAINTEXT_HEX));
            expect(toHex(out)).toBe(CIPHERTEXT_HEX);
        });

        it('decrypts a known ciphertext back to the expected plaintext bytes', async () => {
            const cipher = new KvEncryptionAES256XTSKey(fromHex(KEY_HEX));
            const out = await cipher.decrypt(BLOCK_ID, fromHex(CIPHERTEXT_HEX));
